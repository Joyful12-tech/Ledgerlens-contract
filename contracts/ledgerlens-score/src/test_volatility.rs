...
use crate::{test::initialized, LedgerLensScoreContractClient};
...
fn test_volatility_resets_after_window_expires() {
    ...
    // Submit without the `submit()` helper's trailing 3601s advance — that
    // gap alone would exceed this test's 60s window and reset the state
    // before the second sample lands. Different wallets, so no cooldown
    // clearance is needed between them.
    let w1 = Address::generate(&env);
    client.submit_score(&Vec::new(&env), &w1, &pair, &10, &false, &false, &1, &90, &1, &None);
    let w2 = Address::generate(&env);
    client.submit_score(&Vec::new(&env), &w2, &pair, &90, &false, &false, &1, &90, &1, &None);
    assert!(client.get_pair_volatility(&pair) > 0);
