...
impl LedgerLensScoreContract {
    ...
    /// Alias for `get_finality_buffer`.
    pub fn get_escrow_hold_window(env: Env) -> u64 {
        Self::get_finality_buffer(env)
    }

    /// Alias for `get_pending_score`.
    pub fn get_escrow_score(
        env: Env,
        wallet: Address,
        asset_pair: Symbol,
    ) -> Option<PendingScoreEntry> {
        Self::get_pending_score(env, wallet, asset_pair)
    }

    /// Alias for `cancel_pending_score`.
    pub fn cancel_escrow_score(
        ...
        Self::cancel_pending_score(env, admin_signers, wallet, asset_pair)
    }
    ...
    /// Returns how many times `signer`'s submissions have been rejected as
    /// IQR outliers by `submit_consensus_score` (see #297). `0` if the
    /// signer has never submitted or has never been rejected.
    pub fn get_signer_rejection_count(env: Env, signer: Address) -> u32 {
        storage::get_signer_rejection_count(&env, &signer)
    }

    /// Configures the IQR outlier-rejection multiplier used by
    /// `submit_consensus_score`, scaled ×100 (e.g. `150` = 1.5×). A signer's
    /// score is rejected as an outlier when it deviates from the consensus
    /// median by more than `multiplier/100 × IQR`. Defaults to `150`.
    /// Admin only.
    ...
    /// `set_staleness_window`, or `set_param_change_delay`) once its
    /// time-lock delay has elapsed. Callable by anyone — the only gate is
    /// `apply_after <= now`.
    ///
    /// # Errors
    /// - [`Error::NoPendingParamChange`] if no change is pending for `key`.
    /// - [`Error::ParamChangeNotReady`] if `apply_after > now`.
    pub fn apply_param_change(env: Env, key: Symbol) -> Result<(), Error> {
        ...
        match &pending.new_value {
            ...
            ParamValue::U64(v) if key == symbol_short!("pc_delay") => storage::set_param_change_delay(&env, *v),
    ...
    /// Returns the current delay a pending parameter change must wait before
    /// `apply_param_change` will accept it. Defaults to
    /// `DEFAULT_PARAM_CHANGE_DELAY_SECS` (24h) until configured.
    pub fn get_param_change_delay(env: Env) -> u64 {
        storage::get_param_change_delay(&env)
    }

    /// Proposes a new time-lock delay for `apply_param_change`. Like every
    /// other simple parameter change, this itself goes through the
    /// propose-then-apply flow (keyed `"pc_delay"`), time-locked by the
    /// *current* delay. Admin only.
    ///
    /// # Errors
    /// - [`Error::NotInitialized`] if the contract has no admin yet.
    /// - [`Error::InvalidParamChangeDelay`] if `delay_secs` is outside
    ///   `[MIN_PARAM_CHANGE_DELAY_SECS, MAX_PARAM_CHANGE_DELAY_SECS]`.
    /// - [`Error::ParamChangeAlreadyPending`] if a change for this key is
    ///   already pending.
    ...
    /// Returns the pending parameter change proposal for `key`, if any.
    ///
    /// # Errors
    /// - [`Error::NoPendingParamChange`] if no change is pending for `key`.
    ...
    /// Discards a pending simple parameter change before it can be applied,
    /// freeing the key for a new proposal. Admin only.
    ///
    /// # Errors
    /// - [`Error::NotInitialized`] if the contract has no admin yet.
    /// - [`Error::NoPendingParamChange`] if no change is pending for `key`.
    ...
    pub fn query_risk_gate(
        ...
        Self::charge_gate_query_fee(&env, &wallet);
        Self::query_risk_gate_with_confidence(env, wallet, asset_pair, gate_threshold, 0)
    }

    /// Charges the configured `gate_query_fee` (if any) to `wallet`, silently
    /// skipping when the fee is `0` or no fee token has been configured —
    /// `query_risk_gate` is infallible and must never be blocked by fee setup.
    fn charge_gate_query_fee(env: &Env, wallet: &Address) {
        let fee = storage::get_gate_query_fee(env);
        if fee <= 0 {
            return;
        }
        let Some(token) = storage::get_fee_token(env) else {
            return;
        };
        wallet.require_auth();
        ...
        storage::set_accumulated_fees(env, storage::get_accumulated_fees(env).saturating_add(fee));
    }
    ...
    pub fn set_gate_enforcement_mode(
        ...
        action_data[31] = strict as u8;
        Self::append_governance_action(&env, action_data);
        Ok(())
    }

    /// Appends an admin action to the Merkle governance audit chain (#299):
    /// `new_head = SHA256(0x01 || current_head || action_data)`. Returns and
    /// persists the new head.
    fn append_governance_action(env: &Env, action_data: [u8; 32]) -> BytesN<32> {
        let current = storage::get_governance_chain_head(env);
        let mut buf = [0u8; 65];
        buf[0] = 0x01;
        buf[1..33].copy_from_slice(&current.to_array());
        buf[33..65].copy_from_slice(&action_data);
        let new_head: BytesN<32> = env.crypto().sha256(&Bytes::from_array(env, &buf)).to_bytes();
        storage::set_governance_chain_head(env, &new_head);
        new_head
    }

    /// Returns the current head of the Merkle governance audit chain (#299).
    /// Starts at all-zeros and advances with every recorded admin action.
    pub fn get_governance_chain_head(env: Env) -> BytesN<32> {
        storage::get_governance_chain_head(&env)
    }

    /// Verifies that `action_hash` is (or, via `proof`, chains forward into)
    /// the current governance audit chain head. With an empty `proof`,
    /// `action_hash` must equal the current head directly. Each `proof`
    /// entry is folded forward as the next link's `action_data`
    /// (`acc = SHA256(0x01 || acc || proof[i])`) until the result is
    /// compared against the current head.
    pub fn verify_governance_action(env: Env, action_hash: BytesN<32>, proof: Vec<BytesN<32>>) -> bool {
        let mut acc = action_hash;
        for i in 0..proof.len() {
            let next_data = proof.get(i).unwrap();
            let mut buf = [0u8; 65];
            buf[0] = 0x01;
            buf[1..33].copy_from_slice(&acc.to_array());
            buf[33..65].copy_from_slice(&next_data.to_array());
            acc = env.crypto().sha256(&Bytes::from_array(&env, &buf)).to_bytes();
        }
        acc == storage::get_governance_chain_head(&env)
    }
    ...
    pub fn propose_upgrade(
        ...
        // Approvals accumulate across separate calls, so a single call may
        // legitimately supply fewer than `threshold` signers — unlike the
        // generic `require_admin_auth` gate (used elsewhere) which demands
        // all `threshold` signers up front, here we only need to authenticate
        // whichever signers this call supplied and record them.
        // In legacy (single-admin) mode this whole block is skipped.
    ...
    fn compute_effective_cooldown(env: &Env, asset_pair: &Symbol, base: u64) -> u64 {
        ...
        // effective = base * (1 + (variance_scale/1000) * (norm_var/1000))
        //           = base * (1_000_000 + variance_scale * norm_var) / 1_000_000
        // Using u128 to avoid overflow when base and scale are both large.
        let numerator = 1_000_000u128
            .saturating_add((config.variance_scale as u128).saturating_mul(norm_var as u128));
        let effective = (base as u128).saturating_mul(numerator) / 1_000_000;
    ...
    /// Returns whether the score submission floor policy is currently active.
