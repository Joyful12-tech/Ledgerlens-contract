...
///
/// Kept well below the 1,000 originally specified — combined with
/// `OPS_PER_SEQ` and the per-op invariant check (`NUM_PAIRS * NUM_WALLETS`
/// `get_score` calls), 1,000 sequences means hundreds of thousands of
/// contract invocations, which is too slow for CI.
const SEQUENCES: u32 = 15;
...
fn check_invariants(
    ...
    for (pi, pair) in pairs.iter().enumerate().take(NUM_PAIRS as usize) {
