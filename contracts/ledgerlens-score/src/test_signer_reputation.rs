...
///
/// Needs a third "anchor" submitter: with only two equally-weighted submitters
/// the weighted mean is always exactly their midpoint, so both would always
/// have identical deviation from it by construction, regardless of which
/// values are chosen. A third submitter breaks that forced symmetry.
#[test]
fn test_accuracy_converges_after_deviations() {
    let (env, client) = setup();
    let key = signing_key(2);
    client.set_service_pubkey(&Vec::new(&env), &pubkey_bytes(&env, &key));
    client.set_consensus_config(&3, &20); // wide epsilon
    ...
    let anchor_scores = [60u32, 60, 60];
    let accurate_scores = [58u32, 62, 59];
    let noisy_scores = [75u32, 45, 78];
    ...
    assert_eq!(acc_rec.count, 3);
    assert_eq!(noisy_rec.count, 3);
}
