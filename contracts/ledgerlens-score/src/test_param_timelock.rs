...
fn test_apply_cooldown_after_delay() {
    // set_cooldown applies immediately (see test_cooldown.rs et al.) — it is
    // not part of the propose-then-apply timelock flow, unlike risk_threshold,
    // staleness_window, upgrade_delay, history_max_depth, and param_change_delay.
    let (env, client, _admin, _service) = setup();
