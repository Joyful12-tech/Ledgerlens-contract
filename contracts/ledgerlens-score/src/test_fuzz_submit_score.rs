...
fn fuzz_random_seeds_storage_invariant() {
    ...
    for i in 0..SEEDS {
        ...
        let score = rng.range(0, 110); // occasionally > 100
        let confidence = rng.range(0, 110);
        let timestamp = if rng.range(0, 10) == 0 { 0 } else { now.max(1) };
        let use_tracked = rng.range(0, 5) == 0;
        let wallet = if use_tracked { tracked_wallet.clone() } else { Address::generate(&env) };
        ...
        match result {
            ...
            Ok(Err(_)) => panic!("seed {i}: conversion error on submit"),
            Err(Ok(err)) => {
                ...
                if use_tracked {
                    if let Some(prev) = tracked_score {
                        // Only the rate-limit check is guaranteed to fire when
                        // score/confidence/timestamp are otherwise valid —
                        // an invalid input takes precedence in submit_score's
                        // validation order regardless of cooldown state.
                        let inputs_valid = score <= 100 && confidence <= 100 && timestamp != 0;
                        let should_rate_limit = inputs_valid
                            && last_submit_ts != 0
                        ...
                        let stored = client.get_score(&wallet, &pair);
                        assert_eq!(stored.score, prev, "seed {i}: score changed after rejection");
                    }
                }
